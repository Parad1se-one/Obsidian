...
- Multi-agent Reinforcement Learning with Hybrid Action Space for Free Gait Motion Planning of Hexapod Robots.
  这篇论文主要在解决：CoRL 主会论文索引命中。当前通过 DBLP 检索到该论文，适合作为顶会主会来源补充；详细摘要待后续接入 DOI/全文抓取层。。
  方法上看，它属于 **多智能体 RL** 路线；来源是 `DBLP`，场合/venue 是 `CoRL`。
...
- ComaDICE: Offline Cooperative Multi-Agent Reinforcement Learning with Stationary Distribution Shift Regularization.
  这篇论文主要在解决：ICLR 主会论文索引命中。当前通过 DBLP 检索到该论文，适合作为顶会主会来源补充；详细摘要待后续接入 DOI/全文抓取层。。
...
3. 我今天最建议重点跟的 3 个信号
- Multi-agent Reinforcement Learning with Hybrid Action Space for Free Gait Motion Planning of Hexapod Robots.：RL 正从训练动作策略，转向训练完整智能体闭环。
